...
const cols = 140;
const {data,info} = await sharp(fileURLToPath(new URL('../src/assets/elaina-source.png',import.meta.url))).resize(cols,124,{fit:'fill'}).removeAlpha().raw().toBuffer({resolveWithObject:true});
...
for(let y=0;y<info.height;y++) {
 ...
 for(let x=0;x<cols;x++) {
  ...
  const hair=y>45&&y<110&&b>r*1.03&&b>g&&light>95;
  const layer=hair&&x<55?'left':hair&&x>89?'right':'body';
